const SERVICES = ['Degradê', 'Barba', 'Clássicos', 'Tratamentos']
  ...
  return (
    <main aria-label="Sobre a barbearia" className="px-5 sm:px-10 lg:px-16 py-10 lg:py-16 min-h-[calc(100vh-64px)]">
      <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-12 items-center">
        <div
          className="aspect-[4/5] bg-cream-dark rounded-xl2 border border-line relative overflow-hidden
        ...
        >
          <img
            ...
            className="w-32 h-32 object-contain opacity-90 relative"
          />
          <span aria-hidden="true" className="text-[11px] text-ink-faint tracking-wider uppercase relative">Foto do Tiago</span>
        </div>

        <article>
          <p aria-hidden="true" className="inline-flex items-center gap-2 px-3.5 py-1.5 bg-maroon/[0.1] border border-maroon/25
            rounded-full text-xs font-semibold text-maroon tracking-wide mb-3.5">Sobre</p>
          ...
          <ul aria-label="Especialidades" className="flex gap-2.5 flex-wrap list-none p-0 m-0">
            {SERVICES.map((t) => (
              <li key={t} className="px-3.5 py-1.5 rounded-full bg-navy/[0.08] text-navy text-xs font-semibold">{t}</li>
            ))}
          </ul>
        </article>
      </div>
