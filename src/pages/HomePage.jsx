...
export default function HomePage() {
  ...
  const stats = [
    [t('hero.stat1.valor', '7+'),  t('hero.stat1.label', 'Anos de experiência')],
    [t('hero.stat2.valor', '2k+'), t('hero.stat2.label', 'Clientes satisfeitos')],
    [t('hero.stat3.valor', '5.0'), t('hero.stat3.label', 'Estrelas em reviews')],
  ]
  ...
  return (
    <main>
      <section aria-label="Apresentação e marcação" className="grid lg:grid-cols-[1.1fr_1fr] min-h-[calc(100vh-64px)]">
        ...
        <div className="order-2 justify-center lg:order-1 relative flex flex-col px-6 sm:px-10 lg:px-16 py-10 lg:py-0 bg-cream">
          ...
          <div className="relative max-w-[520px] self-center">
            ...
            <p aria-label={t('hero.badge', 'A aceitar marcações em Braga')}
            ...
            <address className="not-italic bg-paper border border-line rounded-xl2 p-5 shadow-soft animate-fadeUp [animation-delay:.25s]">
              ...
              <div className="grid grid-cols-2 gap-3.5 mb-4">
                <div>
                  ...
                  <p className="text-sm text-ink font-medium leading-snug">
                    {t('contacto.morada1', 'Rua de Exemplo, 25')}<br />
                ...
                <div>
                  ...
                  <p className="text-sm text-ink font-medium leading-snug">
                    <time>{t('contacto.horario.dias', 'Ter–Sáb')}</time><br />
              ...
              <div className="flex items-center justify-between flex-wrap gap-3">
                <a href={`tel:${t('contacto.telefone.href', '+351912345678')}`}
                  aria-label={`Ligar para ${t('contacto.telefone', '+351 912 345 678')}`}
                  className="flex items-center gap-1.5 text-navy font-semibold text-sm">
                  <PhoneIcon /> {t('contacto.telefone', '+351 912 345 678')}
                </a>
                <div className="flex gap-1.5" role="list" aria-label="Redes sociais">
                  {Object.entries(SOCIAL_PATHS).map(([name, d]) => (
                    <a key={name} role="listitem"
                      href={socials[name.toLowerCase()] || '#'}
                      aria-label={`${name} da ${t('hero.titulo', 'Barbearia Tiago Fernandes')}`}
          ...
          <div className="w-full max-w-[480px] mx-auto bg-paper border border-line rounded-xl2 p-6 sm:p-7 shadow-lift animate-fadeUp [animation-delay:.1s]">
            <div className="mb-4">
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span aria-hidden="true" className="text-[11px] font-bold tracking-[0.1em] uppercase text-maroon">Marcar</span>
              </div>
