    ...
    <main>
      <section className="grid lg:grid-cols-[1.1fr_1fr] min-h-[calc(100vh-64px)]">
        ...
        <div className="order-2 lg:order-1 relative flex flex-col justify-center
          px-6 sm:px-10 lg:px-16 py-10 lg:py-0 bg-cream lg:border-r border-line">
          ...
          <div className="relative max-w-[520px]">
            ...
            <div className="flex items-center gap-4 mt-6 mb-4 animate-fadeUp [animation-delay:.05s]">
              <img src="/logo.png" alt="Logo" className="h-20 w-20 object-contain shrink-0" />
              <h1 className="text-[clamp(30px,4.5vw,52px)] font-extrabold leading-[1.02] tracking-tight text-navy">
                Barbearia<br />
                <span className="text-maroon italic font-bold">Tiago Fernandes</span>
              </h1>
            </div>

            <p className="text-[clamp(15px,1.6vw,17px)] text-ink-soft leading-relaxed max-w-[440px] mb-8
              animate-fadeUp [animation-delay:.15s]">
              {tagline}
            </p>
            ...
            <div className="bg-paper border border-line rounded-xl2 p-5 shadow-soft animate-fadeUp [animation-delay:.25s]">
              <div className="flex items-center gap-2 mb-3.5">
                ...
                <span className="text-[11px] font-bold tracking-[0.1em] uppercase text-navy">Contacto & Horário</span>
              </div>
              <div className="grid grid-cols-2 gap-3.5 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-ink font-medium leading-snug">Ter–Sáb<br />9h–12h · 13h–19h</p>
                </div>
              </div>
              <div className="h-px bg-line mb-4" />
              <div className="flex items-center justify-between flex-wrap gap-3">
                <a href={`tel:${BUSINESS.phoneHref}`} className="flex items-center gap-1.5 text-navy font-semibold text-sm">
                  <PhoneIcon /> {BUSINESS.phone}
                </a>
                <div className="flex gap-1.5">
                  {Object.entries(SOCIAL_PATHS).map(([name, d]) => (
                    <a key={name} href={BUSINESS.socials[name.toLowerCase()] || '#'} title={name}
            ...
            <div className="flex gap-8 mt-8 animate-fadeUp [animation-delay:.35s]">
              {[['7+', 'Anos'], ['2k+', 'Clientes'], ['5.0', '★ Reviews']].map(([n, l]) => (
                <div key={l}>
                  <div className="text-2xl font-extrabold text-navy tracking-tight">{n}</div>
                  <div className="text-[11px] text-ink-faint font-semibold tracking-wider uppercase mt-0.5">{l}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* DIREITA — widget de marcação. Em mobile aparece primeiro (order-1) */}
        <div className="order-1 lg:order-2 flex flex-col justify-center bg-cream-dark
          px-5 sm:px-10 lg:px-12 py-7 lg:py-12">
          <div className="w-full max-w-[480px] mx-auto bg-paper border border-line rounded-xl2
            p-6 sm:p-7 shadow-lift animate-fadeUp [animation-delay:.1s]">
            <div className="mb-4">
              <div className="flex items-center gap-2 mb-1.5">
                <span className="inline-block w-8 h-0.5 bg-maroon rounded-full" />
                <span className="text-[11px] font-bold tracking-[0.1em] uppercase text-maroon">Marcar</span>
              </div>
              <h2 className="text-[22px] font-extrabold text-navy tracking-tight leading-tight">
                Marca o teu corte<br />em 3 passos
              </h2>
            </div>
            <BookingWidget user={user} onRequireLogin={onRequireLogin} onBooked={onBooked} />
          </div>
        </div>
      </section>
    </main>
