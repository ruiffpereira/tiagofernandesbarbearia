...
export default function AuthModal({ onClose, onSuccess }) {
  ...
  return (
    <Modal title={mode === 'login' ? 'Bem-vindo de volta' : 'Criar conta'} onClose={onClose}>
      <Button variant="surface" disabled className="w-full opacity-50 cursor-not-allowed">
        <GoogleIcon /> Continuar com Google (brevemente)
      </Button>

      <div className="flex items-center gap-3 text-[11px] tracking-wider uppercase text-ink-faint font-semibold my-4">
        ...
        {mode === 'register' && (
          <>
            <div className="flex flex-col gap-1.5">
              ...
              <Input type="text" placeholder="O teu nome" value={form.name} onChange={(e) => set('name', e.target.value)} required />
            </div>
            <div className="flex flex-col gap-1.5">
              <Label>Telemóvel</Label>
              <Input type="tel" placeholder="+351 9XX XXX XXX" value={form.phone} onChange={(e) => set('phone', e.target.value)} />
            </div>
          </>
        )}
        <div className="flex flex-col gap-1.5">
      ...
      <div className="text-center mt-4 text-[13px] text-ink-soft">
        {mode === 'login' ? (
          <>Sem conta?{' '}
            <button onClick={() => setMode('register')} className="text-navy font-semibold underline">Regista-te</button>
          </>
        ) : (
          <>Já tens conta?{' '}
            <button onClick={() => setMode('login')} className="text-navy font-semibold underline">Entra</button>
          </>
        )}
      </div>
    </Modal>
